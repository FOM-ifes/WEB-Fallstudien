...
# Erste Datenübersicht Mathematik 1
inspect(Mathe1)
# Erste Datenübersicht Mathematik 2
inspect(Mathe2)
```

Während die Klausur Mathematik 1 von $n_1=`r nrow(Mathe1)`$ Studierenden geschrieben wurde, waren es in der Klausur Mathematik 2  $n_2=`r nrow(Mathe2)`$.

*Hinweis:* Man erkennt, dass beide Klausuren gut ausgefallen sind: die Mittelwerte liegen über dem langfristigen Mittelwert der beiden Klausuren.^[Ob es ein besonders guter Kurs, eher einfache Klausuren oder besonders gute Lehre war, weiß der Autor leider nicht mehr.]
...
Die visuellen Eindrücke werden durch die deskriptiven Kennzahlen untermauert:

```{r}
favstats(~ Punkte_M1, data = Mathepunkte)
favstats(~ Punkte_M2, data = Mathepunkte)
...
Gleichzeitig gilt aber: $\hat{\beta_1}=`r round(coef(erglm)[1],2)`<1$, d.h. dass die (Punkt-)Prognose für einen Studierenden mit $z_i^{\text{Mathematik 1}}=1>0$ lautet $\hat{z}_i^{\text{Mathematik 2}}=`r round(coef(erglm)[1],2)` \cdot 1=`r round(coef(erglm)[1],2)`$, also (relativ) *schlechter*, für einen mit $z_i^{\text{Mathematik 1}}=-1<0$ lautet $\hat{z}_i^{\text{Mathematik 2}}=`r round(coef(erglm)[1],2)` \cdot (-1)=-`r round(coef(erglm)[1],2)`$, also (relativ) *besser*.
...
Regression zur Mitte: es ergibt sich mit $\hat{\beta_1}=`r round(coef(erglm)[2],2)`<0$ ein negativer Zusammenhang. 
Um dies Ergebnis kritisch zu hinterfragen wird $\beta_1=0$ simuliert und die gerichtete Hypothese $H_0: \beta_1 \geq 0$ gegen $H_A: \beta_1<0$ geprüft:

```{r}
set.seed(1896)
NullvtlgReg <- do(10000) * lm(diffM2M1 ~ shuffle(Punkte_M1), data = Mathepunkte)
# Verteilung unter H_0
gf_histogram( ~ Punkte_M1, data = NullvtlgReg) %>%
  gf_vline(xintercept = ~ coef(erglm)[2])
# Kritischer Wert (gerichtet)
quantile( ~ Punkte_M1, prob = 0.05, data = NullvtlgReg)
# p-Wert (gerichtet)
prop( ~ (Punkte_M1 < coef(erglm)[2]), data = NullvtlgReg)
```

Die in der Stichprobe beobachtete (negative) Steigung ist also sehr unwahrscheinlich, wenn wir annehmen, dass es keinen Zusammenhang gibt: in keiner der $10000$ Simulationen kam der Wert der Stichprobe vor, daher p-Wert<$1/10000$.
