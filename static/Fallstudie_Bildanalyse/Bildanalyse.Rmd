...
Es handelt sich um ein 3-dimensionales `array`: $`r dimension[2]` \times `r dimension[1]`$ Pixel mit  $`r dimension[3]`$ Farben: Rot, Gelb, Blau.


In Datenmatrix überführen:
```{r}
img_rgb <- data.frame(
  x = rep(1:dimension[2], each = dimension[1]), # Bildzeile 
  y = rep(dimension[1]:1, dimension[2]), # Bildspalte
